...
_LOG_PREFIX = '[LOB-API-CLIENT]'


class LobApiSoccerClient(base_client.ApiBaseClient):

    NUMBER_OF_FETCH_DAYS = None
    API_URL_EVENTS = settings.DIRECT_CLIENT_SPORT_URLS[
        bet_place_enums.BettingInstitutions.LOB.name][betting_enums.Sports.FOOTBALL.name]['url']

    def __init__(self):
        super(LobApiSoccerClient, self).__init__()
    ...
    def _parse_league_matches_base_bet(cls):
        raw_match_events_base = cls._get_raw_league_matches_per_bet(direct_constants.LOB_BASE_ODDS_CODE)
    ...
    def _parse_league_matches_double_chance_bet(cls, base_odds):
        raw_match_events_base = cls._get_raw_league_matches_per_bet(direct_constants.LOB_DOUBLE_CHANCE_CODE)
